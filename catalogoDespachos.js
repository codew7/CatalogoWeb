...
function cargarGrid(data) {
    ...
    data.forEach(item => {
        ...
        valorPesos.innerHTML = `<strong>Valor $</strong> <span class="precio-anterior-cifra">${item[VALOR_CONSUMIDOR]}</span>`;
        valorPesos.style.margin = '10px 0 5px 0';
        card.appendChild(valorPesos);

        // 6. Oferta Interior
        const valorOferta = document.createElement('p');
        valorOferta.innerHTML = `<strong>OfertaInterior</strong> ${item[OFERTA_INTERIOR]}`;
        valorOferta.style.margin = '0 0 15px 0';
        valorOferta.style.fontSize = '15px';
        valorOferta.style.color = '#F57C00';
        card.appendChild(valorOferta);
...
function actualizarCarrito() {
    ...
    if (cartDiscountNote) {
        cartDiscountNote.style.display = 'none';
    }
...
// Modal final del checkout · confirma el pedido y le muestra al cliente dónde
// consultarlo. Ya no se anuncia el email: la vía visible desde el catálogo es
// el botón "Pedidos" (barra inferior en móvil, navegación del header en
// escritorio), así que el modal lo dibuja incrustado en la frase —la réplica se
// reconoce antes que la palabra— y al cerrarse enciende el botón real con
// resaltarBotonPedidos(). Además el modal entra desplegándose desde ese botón
// (ver anclarModalEnBotonPedidos) y se repliega sobre él al cerrarse.
// Los estilos están en catalogo.html, sección 13.1.
function mostrarModalPedidoConfirmado(pedidoId) {
    ...
    overlay.setAttribute('aria-labelledby', 'exitoTitulo');
    overlay.innerHTML = `
        <div class="modal-content exito-modal">
            <button type="button" class="modal-close" id="cerrarExitoBtn" aria-label="Cerrar">
                <i class="fas fa-xmark" aria-hidden="true"></i>
            </button>

            <div class="exito-cabecera">
                <div class="exito-sello" aria-hidden="true">
                    <svg viewBox="0 0 24 24"><path class="exito-tilde" d="M5 12.6 L10 17.6 L19 6.8"/></svg>
                </div>
                <h2 id="exitoTitulo">¡Pedido confirmado!</h2>
                <p>Lo recibimos y ya está en preparación.</p>
            </div>

            <div class="exito-cuerpo">
                <div class="exito-guia">
                    <span class="exito-guia-titulo">
                        <i class="fas fa-location-arrow" aria-hidden="true"></i> Dónde seguirlo
                    </span>
                    <p>
                        Entrá cuando quieras a
                        <span class="exito-chip"><i class="fas fa-receipt" aria-hidden="true"></i>Pedidos<span class="exito-chip-punto" aria-hidden="true"></span></span>
                        y vas a ver el detalle, el estado y el total de tu pedido.
                    </p>
                    <div class="exito-donde">
                        <i class="fas fa-arrow-down-long exito-solo-movil" aria-hidden="true"></i>
                        <i class="fas fa-arrow-up-long exito-solo-escritorio" aria-hidden="true"></i>
                        <span class="exito-solo-movil">Lo tenés en la barra de abajo, a la derecha.</span>
                        <span class="exito-solo-escritorio">Lo tenés arriba, en el menú del catálogo.</span>
                    </div>
                </div>

                <div class="exito-acciones">
                    <button type="button" id="verPedidoBtnConfirmado" class="btn-primary">
                        <i class="fas fa-receipt" aria-hidden="true"></i> Ver mi pedido
                    </button>
                    <button type="button" id="seguirComprandoBtn" class="btn-secondary">
    ...
    // El anclaje va después de insertarlo: recién en el documento el panel tiene
    // medidas con las que calcular desde dónde crece.
    const panel = overlay.querySelector('.exito-modal');
    anclarModalEnBotonPedidos(panel);
    ...
    // Cerrar por la X, por "Seguir comprando" o por el velo termina siempre en
    // el mismo gesto: el modal se repliega sobre el botón del que salió y, una
    // vez fuera de la pantalla, ese botón queda señalado. La bandera evita que
    // dos cierres seguidos (la X y el velo en el mismo gesto) encadenen dos
    // resaltados.
    let cerrando = false;
    function cerrarModalExito() {
        ...
        overlay.classList.add('is-cerrando');
        panel.classList.add('is-cerrando');

        // Acompaña a exito-plegar / fadeOut (.26s en catalogo.html, sección
        // 13.1): si cambia la duración de una, cambiar la de la otra.
        setTimeout(function() {
            overlay.remove();
            resaltarBotonPedidos();
        }, 260);
    }

    document.getElementById('cerrarExitoBtn').addEventListener('click', cerrarModalExito);
    document.getElementById('seguirComprandoBtn').addEventListener('click', cerrarModalExito);
    overlay.addEventListener('mousedown', function(e) {
        if (e.target === overlay) cerrarModalExito();
    });
}

// Hace que el modal se despliegue desde el botón "Pedidos" en vez de aparecer
// en el centro de la nada: le escribe al panel un transform-origin ubicado en el
// centro del botón (en coordenadas relativas al propio panel, que es lo que
// espera la propiedad), y la animación exito-desplegar hace el resto.
// La posición se mide en el momento y no se fija por media query porque el botón
// cambia de lugar según el dispositivo: barra inferior en móvil, navegación del
// header en escritorio. Se toma el primero que esté realmente a la vista
// —getClientRects() queda vacío para el que el CSS oculta—; si no hay ninguno,
// el modal cae en la entrada estándar de los modales.
function anclarModalEnBotonPedidos(panel) {
    const boton = Array.from(document.querySelectorAll('[data-accion="pedidos"]'))
        .find(b => b.getClientRects().length > 0);

    if (!boton) {
        panel.classList.add('exito-sin-ancla');
        return;
    }

    const caja = boton.getBoundingClientRect();
    const marco = panel.getBoundingClientRect();   // sin transformar: el estado base del panel no anima

    panel.style.transformOrigin =
        (caja.left + caja.width / 2 - marco.left) + 'px ' +
        (caja.top + caja.height / 2 - marco.top) + 'px';
    panel.classList.add('exito-desde-boton');
}

// Enciende el botón "Pedidos" real —el de la barra inferior y el del header
// comparten data-accion, así que se marcan los dos y siempre queda señalado el
// que esté a la vista— y lo apaga solo. La espera acompaña a la animación de
// .is-destacado (catalogo.html, sección 13.1: 1.3s × 3): si cambia una, cambiar
// la otra, o la clase queda puesta sin nada que mostrar.
function resaltarBotonPedidos() {
    const botones = document.querySelectorAll('[data-accion="pedidos"]');
    if (!botones.length) return;

    botones.forEach(b => b.classList.add('is-destacado'));
    setTimeout(function() {
        botones.forEach(b => b.classList.remove('is-destacado'));
    }, 4000);
}
...
// Resumen del carrito en un solo lugar: unidades y total.
    ...
    overlay.innerHTML = `
        <div class="modal-content" style="max-width:440px;">
            ...
            <div class="modal-body">
                <div class="minimo-requisitos">
                    <div class="minimo-destacado">
                        <i class="fas fa-wallet" aria-hidden="true"></i>
                        <span class="minimo-texto">
                            <span class="minimo-label">Compra mínima</span>
                            <span class="minimo-monto">$90.000</span>
                        </span>
                    </div>
                    <div class="minimo-destacado">
                        <i class="fas fa-boxes-stacked" aria-hidden="true"></i>
                        <span class="minimo-texto">
                            <span class="minimo-label">Cantidad mínima</span>
                            <span class="minimo-monto">3 unidades</span>
                        </span>
                    </div>
                </div>
                <p class="minimo-nota">
                    Agregá más artículos para alcanzar estos mínimos y seguir con tu pedido.
                    Las 3 unidades pueden ser <strong>iguales o distintas</strong>.
                </p>
            </div>
...
function _enviarPedidoFinalConfirmado() {
    ...
    db.ref('clientes').orderByChild('email').equalTo(emailCliente.toLowerCase()).limitToFirst(1).once('value')
        ...
        .then((pedidoId) => {
            ...
            mostrarModalPedidoConfirmado(pedidoId);
