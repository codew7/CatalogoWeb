...
// === OPTIMIZACIÓN: Caché para evitar consultas redundantes ===
let cacheClientes = new Map(); // Caché de clientes para evitar consultas repetidas
let cachePedidosAbiertos = new Map(); // Caché de pedidos abiertos
let cacheUltimosPedidos = new Map(); // Caché de últimos pedidos
...
if (modoEdicion) {
    ...
    // OPTIMIZACIÓN 4: Cargar solo los datos del cliente (no todo el pedido)
    db.ref('pedidos/' + pedidoEditId + '/cliente').once('value')
        .then(snapshot => {
            if (snapshot.exists()) {
                // Solo guardar los datos del cliente del pedido existente
                datosExtraCliente = snapshot.val();
                console.log('Datos del cliente cargados (optimizado):', datosExtraCliente);
            } else {
                console.log('No se encontraron datos del cliente para este pedido');
            }
        })
        .catch(error => {
        ...
        function aplicarFiltros() {
            ...
            if (query.trim() !== '') {
                filtrados = filtrados.filter(item => 
                    ...
                    item[2].toLowerCase().includes(query) || // Código
...
function enviarPedido() {
    ...
    // Validar que el total del carrito sea al menos $100,000 (no en modo edición)
...
function enviarPedidoFinal() {
    ...
    if (modoEdicion && pedidoEditId) {
        ...
        pedidoRef.once('value').then(snapshot => {
            ...
            if (pedido.locked) {
                alert("Este pedido está cerrado y no se puede editar.");
                return;
            }
            
            // Fusionar artículos: si el artículo ya existe, sumar cantidad; si no, agregarlo
            carrito.forEach(sel => {
                const idx = pedido.items.findIndex(it => it.nombre === sel.nombre);
                if (idx > -1) {
                    // El artículo ya existe, sumar la cantidad
                    pedido.items[idx].cantidad += sel.cantidad;
                } else {
                    // El artículo no existe, agregarlo
                    pedido.items.push({
                        nombre: sel.nombre,
            ...
            // OPTIMIZACIÓN 8: Actualización condicional - solo actualizar lo necesario
            const updates = {
                items: pedido.items,
                cliente: pedido.cliente
            };
            
            // Solo actualizar adminViewed si realmente cambió (era true)
            if (pedido.adminViewed === true) {
                updates.adminViewed = false;
            }
            
            // Solo actualizar lastUpdated si han pasado más de 5 minutos desde la última actualización
            const cincoMinutos = 5 * 60 * 1000; // 5 minutos en milisegundos
            if (!pedido.lastUpdated || (Date.now() - pedido.lastUpdated) > cincoMinutos) {
                updates.lastUpdated = Date.now();
            }
            
            return pedidoRef.update(updates);
        })
        .then(() => {
...
buscarUltimoPedidoBtn.addEventListener('click', function() {
  ...
  if (cacheUltimosPedidos.has(email)) {
    const pedidoCacheado = cacheUltimosPedidos.get(email);
    if (pedidoCacheado && pedidoCacheado.id) {
      mostrarMensajeBusqueda('success', '¡Pedido encontrado! Redirigiendo...');
      setTimeout(() => {
        window.open(`pedidos.html?id=${pedidoCacheado.id}`, '_blank');
        ultimoPedidoModal.style.display = 'none';
        ocultarMensajeBusqueda();
        ultimoPedidoEmailInput.value = '';
      }, 800);
  ...
  // OPTIMIZACIÓN 2: Buscar más eficientemente usando orderBy timestamp y limitToLast
  // Primero verificamos si existen pedidos para este email
  db.ref('pedidos').orderByChild('cliente/email').equalTo(email).limitToFirst(1).once('value')
    .then(verificacionSnap => {
      ...
      // Si existen pedidos, buscar el más reciente de manera más eficiente
      // Crear una consulta compuesta para obtener solo el último pedido
      return db.ref('pedidos').orderByChild('cliente/email').equalTo(email).once('value');
    })
    .then(snap => {
      if (!snap) return; // Salir si la primera consulta falló
      ...
      const ultimo = pedidos.reduce((mas_reciente, actual) => {
        return (!mas_reciente || actual.timestamp > mas_reciente.timestamp) ? actual : mas_reciente;
      }, null);
      
      if (ultimo && ultimo.id) {
        // OPTIMIZACIÓN 2: Guardar en caché el resultado
        cacheUltimosPedidos.set(email, ultimo);
